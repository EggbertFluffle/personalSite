...
<p>By re-arranging <code>y = mx + b</code> to be <code>x = (y - b) / m</code> it&#39;s possible to get the x value for a line by inputting the y values. So by using this, and handling an exception for vertical lines with a slope of <code>undefined</code>,  the line drawing algorithm is done and looks like this.</p>
<pre><code class="language-typescript">line(p1: math.Matrix, p2: math.Matrix): void &rbrace;
    let deltaY: number = p2.get([1]) - p1.get([1]); // y2 - y1
    let deltaX: number = p2.get([0]) - p1.get([0]); // x2 - x1
    let m: number = dy / dx;
